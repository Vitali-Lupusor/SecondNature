...
async function createGroup(subscriptionStartDate, db) {   
   const groupCollection = db.collection('groups')
   let group = await groupCollection.findOne({startDate: subscriptionStartDate})   
   let groupId = ''
   ...
   if (group){
      ...
      const groupInfo = await groupCollection.insertOne(
         {
            startDate: subscriptionStartDate,
...
async function createExampleUsers(numberOfUsers, dateInScaledTime, db) {   

   for ( var i = 0; i < numberOfUsers; i++ ) 
   {
      const userSignUpDate = dateInScaledTime      
      const {subscriptions, subscriptionStartDate} = generateSubscription(userSignUpDate)
      const groupId = await createGroup(subscriptionStartDate, db)
...
module.exports = async (realTimeStartDate, fakeCoStartDate, timeScaleFactor, timer) => {
   ...
   try {      
      ...
      await createExampleUsers(numberOfUsersToCreate, dateInScaledTime, db)    
